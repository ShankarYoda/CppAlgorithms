  ...
  for (int i = 0; i <= length1; i++)
    ...
    for (int j = 0; j <= lenght2; j++)
	{
	    if (i == 0 || j == 0)
		 ...
		 {
	      int a = dp[i - 1][j];	// when first Character of first string is
		                        //considered in the Longest Common Sequency solution .
          int b = dp[i][j - 1];	// when first Character of second string is
			                    //considered in the Longest Common Sequency solution .*/
